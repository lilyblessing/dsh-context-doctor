...
function updatedLabel(refreshedAt, t) {
    if (refreshedAt === null)
        return '—';
...
export function ContextAuditRing(props) {
    ...
    return _jsxs("span", { "data-context-doctor": true, style: dockStyle, children: [_jsxs("button", { type: "button", onClick: () => setOpen(value => !value), title: t('cd.title'), "aria-label": t('cd.title'), "aria-expanded": open, "aria-controls": panelId, style: triggerStyle, children: [_jsx("span", { style: { color: accent, display: 'inline-flex' }, children: _jsx(PulseIcon, { size: 17 }) }), _jsx("span", { style: triggerLabelStyle, children: t('cd.title') }), _jsx("span", { "aria-hidden": "true", style: { ...triggerStatusStyle, background: accent } })] }), open && _jsxs("section", { id: panelId, role: "dialog", "aria-label": t('cd.title'), style: panelStyle, children: [_jsxs("header", { style: headerStyle, children: [_jsx("span", { style: { color: TONE.mint, display: 'inline-flex' }, children: _jsx(PulseIcon, { size: 31 }) }), _jsxs("div", { children: [_jsx("h2", { style: titleStyle, children: t('cd.title') }), _jsx("p", { style: subtitleStyle, children: t('cd.residentTokens') })] })] }), state.state === 'error' && _jsx("div", { style: errorStyle, children: t('cd.errorDetail', { error: state.error ?? '' }) }), report === null && state.state !== 'error' ? _jsx("div", { style: emptyStyle, children: state.state === 'loading' ? t('cd.loading') : t('cd.empty') }) : report !== null && _jsxs(_Fragment, { children: [_jsxs("div", { style: summaryStyle, children: [_jsxs("div", { style: gaugeColumnStyle, children: [_jsxs("div", { style: gaugeWrapStyle, children: [_jsx(BudgetRing, { percent: percent, color: accent }), _jsxs("div", { style: gaugeCaptionStyle, children: [_jsxs("strong", { style: { color: accent, fontSize: 37, fontWeight: 460 }, children: [Math.round(Math.min(percent, 1) * 100), "%"] }), _jsx("span", { style: gaugeGuideStyle, children: t('cd.guideline') })] })] }), _jsx("span", { style: totalLabelStyle, children: t('cd.total') }), _jsx("strong", { style: totalStyle, children: formatK(resident) }), _jsx("span", { style: tokensStyle, children: t('cd.tokens') })] }), _jsxs("div", { style: metricsStyle, children: [_jsx(MetricRow, { type: "instructions", label: t('cd.instructions'), value: instructions, ratio: resident === 0 ? 0 : instructions / resident, color: TONE.mint }), _jsx(MetricRow, { type: "skills", label: t('cd.skills'), value: skills, ratio: resident === 0 ? 0 : skills / resident, color: TONE.mint }), _jsx(MetricRow, { type: "tools", label: t('cd.tools'), value: schemas, ratio: resident === 0 ? 0 : schemas / resident, color: TONE.amber }), _jsx(MetricRow, { type: "mcp", label: t('cd.mcp'), value: report.injected.tools.mcp.totalTokens, ratio: resident === 0 ? 0 : report.injected.tools.mcp.totalTokens / resident, color: TONE.mint })] })] }), _jsxs("div", { style: healthStyle, children: [_jsx("span", { style: { color: accent, display: 'inline-flex' }, children: _jsx(CheckIcon, {}) }), _jsxs("div", { children: [_jsx("strong", { style: { ...healthTitleStyle, color: accent }, children: status }), _jsx("p", { style: healthCopyStyle, children: suggestions.length ? t('cd.reviewHint') : t('cd.healthyHint') })] })] }), suggestions.length > 0 && _jsxs("div", { style: suggestionsStyle, children: [_jsx("h3", { style: suggestionsTitleStyle, children: t('cd.suggestions', { n: suggestions.length }) }), _jsx("ol", { style: suggestionListStyle, children: suggestions.slice(0, 3).map((suggestion, index) => {
                                            const tone = suggestion.severity === 'high' ? TONE.red : suggestion.severity === 'medium' ? TONE.amber : TONE.mint;
                                            return _jsxs("li", { style: suggestionStyle, children: [_jsx("span", { style: { ...suggestionIndexStyle, color: tone, borderColor: tone }, children: index + 1 }), _jsxs("span", { style: suggestionCopyStyle, children: [_jsx("strong", { style: { color: tone, fontWeight: 520 }, children: t('cd.auditFinding') }), _jsx("small", { children: suggestion.text })] }), _jsx("span", { "aria-hidden": "true", style: arrowStyle, children: "\u203A" })] }, `${suggestion.severity}-${suggestion.text}`);
                                        }) })] })] }), _jsxs("footer", { style: footerStyle, children: [_jsxs("span", { style: updatedStyle, children: [t('cd.updated'), ": ", updatedLabel(state.refreshedAt, t)] }), _jsxs("button", { type: "button", onClick: refresh, disabled: state.state === 'loading', style: refreshStyle, children: [_jsx(RefreshIcon, {}), t('cd.refresh')] })] })] })] });
}
